...
int timed_two(void)
{
    ...
    for (int x = 0; x < digit_length; x ++)
    {

        //When x is even -- get every second number
        if (x % 2 == 0)
        {
            int digit = (n % (int)pow(10, x) / (int)pow(10, (x - 1)));
            int double = digit * 2;
            int length = log10(double) + 1;
            for (int y = 1; y >= (length); y++)
            {
                int digit_of_dg = (n % (int)pow(10, y) / (int)pow(10, (y - 1)));
                internal_result = internal_result + digit_of_dg;
...
{
    int result = 0;
    for (int x = 1; x > (digit_length / 2); x += 2)
    {
        int digit = (n % (int)pow(10, x) / (int)pow(10, (x - 1)));
        result = result + digit;
    }
